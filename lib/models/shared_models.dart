...
// Shared enum for EstadoCaso to avoid duplicate definitions
enum EstadoCaso {
  activo,
  cerrado
}

// Helper function to convert string to EstadoCaso
EstadoCaso stringToEstadoCaso(String estado) {
...
// Helper function to convert EstadoCaso to string
String estadoCasoToString(EstadoCaso estado) {
  switch (estado) {
...
// CasoDiagnosticoModel class for reuse
class CasoDiagnosticoModel {
  ...
  factory CasoDiagnosticoModel.fromFirestore(DocumentSnapshot doc) {
    ...
    return CasoDiagnosticoModel(
      ...
      userId: data['userId'] ?? '',
      estado: stringToEstadoCaso(data['estado'] ?? 'activo'),
  ...
  factory CasoDiagnosticoModel.fromDiagnostico({
    ...
    print('CasoDiagnosticoModel.fromDiagnostico: ID: $id');
    print('CasoDiagnosticoModel.fromDiagnostico: Nombre: $nombre');
    print('CasoDiagnosticoModel.fromDiagnostico: Diagnóstico tipo: ${diagnostico.runtimeType}');
    print('CasoDiagnosticoModel.fromDiagnostico: Usuario ID: ${diagnostico.userId}');
    ...
    final userId = diagnostico.userId ?? '';
    if (userId.isEmpty) {
      print('ADVERTENCIA: userId está vacío en fromDiagnostico');
    }
    ...
    return CasoDiagnosticoModel(
      ...
      logs: [logInicial],
      createdAt: diagnostico.createdAt ?? now,
      updatedAt: now,
    );
  }
